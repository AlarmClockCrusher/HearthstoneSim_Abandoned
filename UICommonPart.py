...
class GUI_Common:
	...
	def cancelSelection(self):
		if self.UI < 3 and self.UI > -1: #只有非发现状态,且游戏不在结算过程中时下才能取消选择
			...
			for btn in fixedList(self.btnsDrawn):
	...
	def resolveMove(self, entity, button, selectedSubject, info=None):
		...
		if self.UI < 0: pass
			...
			if selectedSubject == "ChooseOneOption" and entity.available():
				...
				for btn in fixedList(self.btnsDrawn):
					if isinstance(btn, ChooseOneButton):
						btn.remove()
						self.btnsDrawn.remove(btn)
	...
	def fatigueAni(self, ID, damage):
		ownID = self.ID if hasattr(self, "ID") else 1
		btn = tk.Button(master=self.GamePanel, text=str(damage), width=HandIconWidth_noImg, height=HandIconHeight_noImg, font=("Yahei", 15, "bold"))
	...
	def cardEntersHandAni_2(self, btn, i, steps): #Add the card to the correct pos in handZone btnsDraw
