...
class Server:
	def __init__(self, address="127.0.0.1", numTables=2):
		...
		self.setaPort4TableQuery()
		self.setTables4Players(numTables)
		self.tables = {}
		
	def setaPort4TableQuery(self):
	...
	def setTables4Players(self, numTables):
		...
		for sock in self.socketsAvailable: print("\t", sock)
		
	#Remove a (port, sock) from self.socketsAvailable, and add (sock, conn, port) to scoketsOccupied
	def guestJoinsaTable(self, sock, conn, addr):
	...
	def findSocketfromConn(self, conn):
		return next(s for s, connection, port in self.socketsOccupied if connection == conn)
	...
	def tellGuestAvailableTables(self, sock):
		conn, addr = sock.accept()
		print("Table query from address", addr)
		if sock == self.socket4PortQuery: #如果是询问还有什么port可以用,无论如何最后都要关闭这个conn
			if self.socketsAvailable:
				...
				for port, sock in self.socketsAvailable:
					response += b",%d"%port
				print("Returns available tables to client", response)
			...
			print("A guest joins table:", sock, conn, addr)
			self.guestJoinsaTable(sock, conn, addr) #请求连接到一个port for player
			
	def handleStartorJoinTable(self, conn, tableID, heroPickle): #data:"Start/Join a Table||gwoiegwog||heroPickle" #Table ID是一个随机字符串
		if tableID in self.tables: #Join a table, after checking if the table already has two players
			...
			table.heroes[1] = heroPickle
			table.infos4Players[1] = b"Table reserved. Wait for 2nd player to join" #while confirming cardpool||"+bytes(table.boardID.encode())
			...
			try:
				...
				for key, mask in events:
					if key.data is None: #客户端尝试连接。如果是询问queryPort,则如果有空余位置时当场返回可用port列表，然后断开； 否则直接断开。
						...
						self.tellGuestAvailableTables(key.fileobj)
						...
						if mask & selectors.EVENT_READ:
							...
							if data: #可能是申请预订或者加入一张桌子，或者是起始阶段双方交换手牌牌库和英雄信息，或者是正常的游戏操作
								if data.startswith(b"Start/Join a Table"):
									action, tableID, heroPickle = data.split(b"||")
									self.handleStartorJoinTable(conn, tableID, heroPickle)
									...
									print("Received the game play from a player", data)
									table, ID = self.findTableandIDwithConn(conn)
									table.infos4Players[3-ID] = data
									print("The conns and data for each player", table.conns2Players, table.infos4Players)
					...
					for ID in [1, 2]:
						if table.conns2Players[ID]:
							try:
								...
								self.freeaTable(table.conns2Players[ID])
