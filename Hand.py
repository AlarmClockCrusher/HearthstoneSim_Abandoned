...
class Hand_Deck:
    ...
    def drawCard(self, ID, card=None, type="Extra"):
        ...
        if self.handNotFull(ID):
            ...
            if cardTracker[0].type == "Spell" and "Casts When Drawn" in cardTracker[0].index:
                ...
                if cardTracker[0].type == "Minion" or cardTracker[0].type == "Amulet" and cardTracker[0].triggers[
                    "Drawn"] != []:
    ...
    def addCardtoHand(self, obj, ID, comment="", byDiscover=False, i=-1):
        ...
        if not isinstance(obj,
                          (list, np.ndarray, tuple)):  # if the obj is not a list, turn it into a single-element list
            obj = [obj]
        morethan3 = len(obj) > 2
        for card in obj:
            if self.handNotFull(ID):
...
Default1 = [HarnessedGlass,HarnessedGlass,HarnessedGlass,HarnessedGlass,
            HarnessedFlame,HarnessedFlame,HarnessedFlame,HarnessedFlame,
            WellofDestiny,WellofDestiny,WellofDestiny]

Default2 = [Fighter, Fighter, Fighter, ElvenArcher, ElvenArcher, ElvenArcher, ElvenArcher, AirboundBarrage,
            AirboundBarrage, AirboundBarrage]
