...
class Trig_ForgeUpgrade(TrigHand):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		...
		for key, value in newCard.effects.items(): #Find keywords the new card doesn't have
			if value < 1 and card.effects[key] > 0: newCard.effects[key] = 1
		for key, value in newCard.effects.items():
			try:
				if value < 1 and card.effects[key] > 0: newCard.effects[key] = 1
			except: pass
...
class TalentedArcanist(Minion):
	...
	name_CN = "精明的奥术师"
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		self.Game.effects[self.ID]["Spell Damage"] += 2
...
class TalentedArcanist_Effect:
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		self.Game.effects[self.ID]["Spell Damage"] -= 1
		self.disconnect()
...
class Trig_SunwellInitiate(Frenzy):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		self.entity.getsEffect("Divine Shield")
...
class Swiftthistle_1(Option):
	type, effect = "Option_Minion", "Rush"
	mana, attack, health = 1, -1, -1
...
class Earthroot_1(Option):
	type, effect = "Option_Minion", "Taunt"
	mana, attack, health = 1, -1, -1
...
class Sungrass_1(Option):
	type, effect = "Option_Minion", "Divine Shield"
	mana, attack, health = 1, -1, -1
...
class Liferoot_1(Option):
	type, effect = "Option_Minion", "Lifesteal"
	mana, attack, health = 1, -1, -1
...
class Fadeleaf_1(Option):
	type, effect = "Option_Minion", "Stealth"
	mana, attack, health = 1, -1, -1
...
class GraveMoss_1(Option):
	type, effect = "Option_Minion", "Poisonous"
	mana, attack, health = 1, -1, -1
...
class LesserGolem__Icecap(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		if targets: npchoice(targets).getsEffect("Frozen")
		return None
...
class GreaterGolem__Icecap(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		if minions:
			...
			for minion in minions: minion.getsEffect("Frozen")
		return None
...
class ShadowHunterVoljin(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if target and not target.dead and target.onBoard: #战吼触发时自己不能死亡。
			...
			if curGame.mode == 0:
				indices = [i for i, card in enumerate(hand) if card.type == "Minion"]
				if indices:
					i = npchoice(indices)
...
class DrawaDeathrattleMinion(Deathrattle_Weapon):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		indices = [i for i, card in enumerate(self.entity.Game.Hand_Deck.decks[self.entity.ID]) if card.type == "Minion" and card.deathrattles]
...
class SummonDeathrattleMinionfromHand_Mana3orLess(Deathrattle_Minion):
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		minion = self.entity
		indices = [i for i, card in enumerate(minion.Game.Hand_Deck.hands[minion.ID]) if card.type == "Minion" and card.deathrattles and card.mana < 4]
		if indices:
			minion.Game.summonfrom(npchoice(indices), minion.ID, minion.pos+1, minion, source='H')
...
class SummonDeathrattleMinionfromHand_Mana4orLess(Deathrattle_Minion):
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		minion = self.entity
		indices = [i for i, card in enumerate(minion.Game.Hand_Deck.hands[minion.ID]) if card.type == "Minion" and card.deathrattles and card.mana < 5]
		if indices:
			minion.Game.summonfrom(npchoice(indices), minion.ID, minion.pos + 1, minion, source='H')
...
class VengefulSpirit(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if posinHand == 0 or posinHand == -1:
			...
			for num in range(2):
				indices = [i for i, card in enumerate(ownDeck) if card.type == "Minion" and card.deathrattles]
...
class DeathSpeakerBlackthorn(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		refMinion = self
		for num in range(3):
			indices = [i for i, card in enumerate(game.Hand_Deck.decks[self.ID]) if card.type == "Minion" and card.deathrattles]
			if indices: refMinion = game.summonfrom(npchoice(indices), self.ID, refMinion.pos + 1, self, source='D')
			else: break
		return None
...
class LivingSeedRank1(Spell):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=0):
		indices = [i for i, card in enumerate(self.Game.Hand_Deck.decks[self.ID]) if card.type == "Minion" and "Beast" in card.race]
		if indices:
			beast = self.Game.Hand_Deck.drawCard(self.ID, npchoice(indices))[0]
...
class MarkoftheSpikeshell(Spell):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if target:
			target.buffDebuff(2, 2)  #buffDebuff() and getsEffect() will check if the minion is onBoard or inHand.
...
class GameManaAura_InTurn1stTaunt2Less(TempManaEffect):
	...
	def applicable(self, target):
		return target.ID == self.ID and target.type == "Minion" and target.effects["Taunt"] > 0
...
class Trig_PlaguemawtheRotting(TrigBoard):
	...
	def canTrig(self, signal, ID, subject, target, number, comment, choice=0):
		return self.entity.onBoard and target != self.entity and target.ID == self.entity.ID and target.effects["Taunt"] > 0#Technically, minion has to disappear before dies. But just in case.
...
class Trig_TavishStormpike(TrigBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		minion = self.entity
		indices = [i for i, card in enumerate(minion.Game.Hand_Deck.decks[ID]) \
						  	if "Beast" in card.race and card.mana == subject.mana - 1]
		if indices and minion.Game.space(minion.ID) > 0:
			minion.Game.summonfrom(npchoice(indices), minion.ID, minion.pos + 1, self, source='D')
...
class BarakKodobane(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		for cost in range(1, 4):
			indices = [i for i, card in enumerate(ownDeck) if card.type == "Spell" and card.mana == cost]
			if indices: self.Game.Hand_Deck.drawCard(self.ID, npchoice(indices))
		return None
...
class FlurryRank1(Spell):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=0):
		minions = self.Game.minionsonBoard(3-self.ID)
		if minions: npchoice(minions).getsEffect("Frozen")
		return None
...
class VardenDawngrasp(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		for minion in targets_Freeze: minion.getsEffect("Frozen")
		if targets_Damage:
...
class KnightofAnointment(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		indices = [i for i, card in enumerate(self.Game.Hand_Deck.decks[self.ID]) if card.school == "Holy"]
		if indices: self.Game.Hand_Deck.drawCard(self.ID, npchoice(indices))
		return None
...
class ParalyticPoison(Spell):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		weapon = self.Game.availableWeapon(self.ID)
		if weapon:
			weapon.buffDebuff(1, 0)
			weapon.getsTrig(Trig_ParalyticPoison(weapon))
		return None
...
class Yoink(Spell):
	...
	def discoverDecided(self, option, case, info_RNGSync=None, info_GUISync=None):
		if case == "Discovered" or case == "Random":
			self.Game.picks_Backup.append((info_RNGSync, info_GUISync, case == "Random",
											option if inspect.isclass(option) else type(option)) )
		option.getsTrig(Trig_SwapBackPowerAfter2Uses(option, self.Game.powers[self.ID]),
						trigType="TrigBoard", connect=False)
...
class Trig_SwapBackPowerAfter2Uses(Trig_Countdown):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		if self.counter < 1:
			self.powerReplaced.ID = self.entity.ID
...
class SilverleafPoison(Spell):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=0):
		weapon = self.Game.availableWeapon(self.ID)
		if weapon: weapon.getsTrig(Trig_SilverleafPoison(weapon))
		return None
...
class Trig_TinyfinsCaravan(TrigBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		indices = [i for i, card in enumerate(self.entity.Game.Hand_Deck.decks[self.entity.ID]) if "Murloc" in card.race]
		if indices: self.entity.Game.Hand_Deck.drawCard(self.entity.ID, npchoice(indices))
...
class Trig_ApothecarysCaravan(TrigBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		minion = self.entity
		indices = [i for i, card in enumerate(minion.Game.Hand_Deck.decks[minion.ID]) if card.type == "Minion" and card.mana == 1]
		if indices: minion.Game.summonfrom(npchoice(indices), minion.ID, minion.pos+1, minion, source='D')
...
class BulkUp(Spell):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		minions = [card for card in self.Game.Hand_Deck.hands[self.ID] if card.type == "Minion" and card.effects["Taunt"] > 0]
		if minions:
...
class SelflessSidekick(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		indices = [i for i, card in enumerate(self.Game.Hand_Deck.decks[self.ID]) if card.type == "Weapon"]
		if indices:
			self.Game.equipWeapon(self.Game.Hand_Deck.extractfromDeck(npchoice(indices), self.ID, animate=False)[0])
		return None
...
class Deal1DamagetoEnemyMinions(Deathrattle_Minion):
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		...
		minion.dealsAOE(targets, [1]*len(targets))
...
class ARandomBeastinYourHandCosts1Less(Deathrattle_Minion):
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		beasts = [card for card in self.entity.Game.Hand_Deck.hands[self.entity.ID] if "Beast" in card.race]
...
class LadyAnacondra(Minion):
	...
	def manaAuraApplicable(self, subject):
		return subject.ID != self.ID and subject.school == "Nature"
...
class FrostweaveDungeoneer(Minion):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		indices = [i for i, card in enumerate(self.Game.Hand_Deck.decks[self.ID]) if card.type == "Spell"]
		if indices:
			spell = self.Game.Hand_Deck.drawCard(self.ID, npchoice(indices))[0]
...
class GiveYourMinionsDivineShield(Deathrattle_Weapon):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		for minion in self.entity.Game.minionsonBoard(self.entity.ID):
			minion.getsEffect("Divine Shield")
...
class SavoryDeviateDelight(Spell):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		if game.mode == 0:
			...
			indices1 = [i for i, card in enumerate(game.Hand_Deck.hands[1]) if card.type == "Minion"]
			indices2 = [i for i, card in enumerate(game.Hand_Deck.hands[2]) if card.type == "Minion"]
			if indices1: i, minion1 = npchoice(indices1), npchoice(pool)
			else: i, minion1 = -1, None
			if indices2: j, minion2 = npchoice(indices2), npchoice(pool)
...
def panda_Show2TransformReturn(game, GUI, initiator, cards, newCards):
	...
	for card, newCard in zip(cards, newCards):
		game.transform(card, newCard, creator=type(initiator))
...
class HasAura_WaterMoccasin:
	...
	def trig(self, signal, ID, subject, target, number, comment, choice=0):
		...
		if otherMinions and self.on:
			...
			self.on = True
			Aura_Receiver(minion, self, effect="Poisonous").effectStart()
...
class ShroudofConcealment(Spell):
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		for num in range(2):
			indices = [i for i, card in enumerate(self.Game.Hand_Deck.decks[self.ID]) if card.type == "Minion"]
			if indices:
				card = self.Game.Hand_Deck.drawCard(self.ID, npchoice(indices))[0]
...
class ShroudofConcealment_Effect:
	...
	def trig(self, signal, ID, subject, target, number, comment, choice=0):
		if self.canTrig(signal, ID, subject, target, number, comment):
			if self.Game.GUI: self.Game.GUI.showOffBoardTrig(self.card)
			subject.getsEffect("Temp Stealth")
			self.cardsDrawn.remove(subject)
			if not self.cardsDrawn: self.disconnect()
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		if indices:
			...
			if spell and spell.school == "Nature":
				indices = [i for i, card in enumerate(ownDeck) if "Elemental" in card.race]
				if indices: self.Game.Hand_Deck.drawCard(self.ID, npchoice(indices))
		return None
...
class Trig_StealerofSouls(TrigBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		target[0].getsEffect("Cost Health Instead")
