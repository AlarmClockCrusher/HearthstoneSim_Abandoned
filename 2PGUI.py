...
class GUI_2P(GUI_Common):
	def __init__(self):
		...
		if var.get() == 1: #点击左边的情况会有要求先换牌，然后把信息传给对方
			self.ID, self.DIYs, self.boardID = 1, [monkVar.get()], self.boardID.get()
			#制作cardPool的同时也会返回真正的boardID
			self.boardID = makeCardPool(self.DIYs, self.boardID)
			...
			if self.pickleFile:
				...
				if move == "DefineGame":
					makeCardPool(self.DIYs, self.boardID)
					from CardPools import Classes, ClassesandNeutral, ClassDict, cardPool, MinionsofCost, RNGPools
					self.window.destroy()
					self.window = tk.Tk()
	...
	def initLoadDeckUI(self):
		...
		self.output = tk.Listbox(master=self.outPutPanel, xscrollcommand=scrollbar_hor.set, yscrollcommand=scrollbar_ver.set, width=30, height=7, bg="white", font=("Yahei", 13))
