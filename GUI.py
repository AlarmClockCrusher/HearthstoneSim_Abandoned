...
class DiscoverOptionButton(HandButton):
	def leftClick(self, event):
		discoverOptions = []
		for button in self.GUI.buttonsDrawn:
			if type(button) == DiscoverOptionButton:
...
class DiscoverHideButton(tk.Button):
	def respond(self):
		...
		if self.selected == 1: #When hide is positive, the discover option buttons are destroyed
			...
			for button in self.GUI.buttonsDrawn:
				if type(button) == DiscoverOptionButton:
					button.place(x=button.pos[0], y=button.pos[1], anchor='c')
...
class MinionButton(tk.Button):
	...
	def plot(self, x=11, y=11, anchor='c'):
		...
		if self.card.triggersonBoard != [] or (hasattr(self.card, "deathrattles") and self.card.deathrattles != []):
			...
			lbl_trigger.plot(x=x, y=y+0.43*CARD_Y, anchor='c')
			self.labels.append(lbl_trigger)
		if self.card.cardType == "Minion":
			attack, attack_Enchant, health, health_Enchant, health_upper = self.card.attack, self.card.attack_Enchant, self.card.health, self.card.health_Enchant, self.card.health_upper
			attColor = "green" if attack_Enchant > self.card.attack_0 else "black"
			lbl_attack = CardLabel(text=str(attack), bg="white", fg=attColor, font=("Yahei", 11, "bold"))
			lbl_attack.plot(x=x-0.39*CARD_X, y=y+0.42*CARD_Y, anchor='c')
			self.labels.append(lbl_attack)
			if health >= health_upper:
				healthColor = "black" if health_Enchant <= self.card.health_0 else "green"
			else:
				healthColor = "red"
			lbl_health = CardLabel(text=str(health), bg="white", fg=healthColor, font=("Yahei", 10))
			lbl_health.plot(x=x+0.39*CARD_X, y=y+0.42*CARD_Y, anchor='c')
			self.labels.append(lbl_health)
...
class HeroButton(tk.Button):
	...
	def plot(self, x=11, y=11, anchor='c'):
		...
		#cardName = self.card.name
		#lbl_name = CardLabel(text=self.GUI.wrapText(cardName), bg="white", fg="black", font=("Yahei", 11, "bold"))
		#lbl_name.plot(x=x, y=y-0.2*CARD_Y, anchor='c')
		#self.labels.append(lbl_name)
		attack, health, health_upper, armor = self.card.attack, self.card.health, self.card.health_upper, self.card.armor
		if attack > 0:
			lbl_attack = CardLabel(text=str(attack), bg="black", fg="white", font=("Yahei", 11, "bold"))
			lbl_attack.plot(x=x-0.39*CARD_X, y=y+0.3*CARD_Y, anchor='c')
			self.labels.append(lbl_attack)
		if armor > 0:
			lbl_attack = CardLabel(text=str(attack), bg="black", fg="white", font=("Yahei", 11, "bold"))
			lbl_attack.plot(x=x+0.39*CARD_X, y=y+0.2*CARD_Y, anchor='c')
			self.labels.append(lbl_attack)
...
class WeaponButton(tk.Button): #休眠物和武器无论左右键都是取消选择，打印目前状态
	...
	def plot(self, x=11, y=11, anchor='c'):
		...
		#cardName = self.card.name
		#lbl_name = CardLabel(text=self.GUI.wrapText(cardName), bg="white", fg="black", font=("Yahei", 11, "bold"))
		#lbl_name.plot(x=x, y=y-0.3*CARD_Y, anchor='c')
		#self.labels.append(lbl_name)
		if self.card.triggersonBoard != [] or (hasattr(self.card, "deathrattles") and self.card.deathrattles != []):
			lbl_trigger = CardLabel(text='  ', bg="yellow", fg="white", font=("Yahei", 6, ))
...
class HeroPowerButton(tk.Button): #For Hero Powers that are on board
	...
	def plot(self, x=11, y=11, anchor='c'):
		...
		lbl_mana.plot(x=x, y=y-0.4*CARD_Y, anchor='c')
		self.labels.append(lbl_mana)
		#lbl_name = CardLabel(text=self.GUI.wrapText(cardName), bg="white", fg="black", font=("Yahei", 11, "bold"))
...
class GUI:
		...
		if self.UI == -1: #Draw the mulligan part, the cards and the start turn button
			...
			if self.UI == 1:
				...
				for i in range(len(self.Game.minions[1])):
					self.posMinionsDrawn[1].append((0.043*X+(X/8)*i, 0.62*Y))
				for i in range(len(self.Game.minions[2])):
					self.posMinionsDrawn[2].append((0.043*X+(X/8)*i, 0.40*Y))
				...
				for i in range(len(self.Game.Hand_Deck.hands[2])):
					self.posHands[2].append((0.043*X+(X/13)*i, 0.09*Y))
	...
	def drawMulligan(self):
		for ID in range(1, 3):
			...
			for i in range(len(self.posMulligans[ID])):
				...
				filepath = "Crops\\%s\\%s.png"%(card.index.split('~')[0], type(card).__name__)
	...
	def drawBoard(self):
		btnBoard = BoardButton(relief=tk.FLAT, master=self.GamePanel, bg=boardColor, width=130, height=19)
	...
	def drawHands(self):
		for ID in range(1, 3):
			for i in range(len(self.posHands[ID])):
				...
				ph = PIL.ImageTk.PhotoImage(img)
				btnHand = HandButton(relief=tk.FLAT, image=ph, master=self.GamePanel, bg=color, width=70, height=100)
				btnHand.image = ph
	...
	def drawMinions(self):
		for ID in range(1, 3):
			for i in range(len(self.posMinionsDrawn[ID])):
				...
				color = "green" if (minion.cardType == "Minion" and minion.canAttack()) else "red"
				...
				if hasattr(minion, "progress"):
					text += "   Count:"+str(minion.progress)
				text += '\n'
				...
				if minion.race != "":
					if ',' not in minion.race:
						...
						text += "All"
				filepath = "Crops\\%s\\%s.png"%(minion.index.split('~')[0], type(minion).__name__)
				img = PIL.Image.open(filepath)
				img = img.resize((78, 75))
				ph = PIL.ImageTk.PhotoImage(img)
	...
	def drawHeroesWeaponsPowers(self):
		for ID in range(1, 3):
			...
			filepath = "Crops\\HerosandPowers\\%s.png"%type(hero).__name__
			img = PIL.Image.open(filepath)
			img = img.resize((78, 75))
			ph = PIL.ImageTk.PhotoImage(img)
			btnHero = HeroButton(relief=tk.FLAT, master=self.GamePanel, image=ph, bg=color, width = CARD_X, height=0.8*CARD_Y)
			btnHero.image = ph
			...
			filepath = "Crops\\HerosandPowers\\%s.png"%type(heroPower).__name__
			img = PIL.Image.open(filepath)
			img = img.resize((70, 70))
			ph = PIL.ImageTk.PhotoImage(img)
			btnHeroPower = HeroPowerButton(relief=tk.FLAT, master=self.GamePanel, image=ph, bg=color, width = 80, height=80)
			...
			if weapon != None:
				...
				if hasattr(weapon, "progress"):
					text += "   Count:"+str(weapon.progress)
				for key, value in weapon.keyWords.items():
					if value > 0:
						text += key+'\n'
				filepath = "Crops\\%s\\%s.png"%(weapon.index.split('~')[0], type(weapon).__name__)
				img = PIL.Image.open(filepath)
				img = img.resize((78, 75))
				ph = PIL.ImageTk.PhotoImage(img)
				btnWeapon = WeaponButton(text=text, relief=tk.FLAT, image=ph, master=self.GamePanel, bg="blue", width = 80, height=80, font=("Yahei", 10, "bold"))
				btnWeapon.image = ph
	...
	def drawManasHandsDecksSecretsQuests(self):
		...
		lbl_Mana1.place(relx=0.9,rely=0.65, anchor='c')
		lbl_Mana2.place(relx=0.9,rely=0.35, anchor='c')
	...
	def drawChooseOne(self):
		...
		for i in range(len(self.Game.options)):
			...
			btnChooseOne = ChooseOneButton(relief=tk.FLAT, master=self.GamePanel, text=option.name, bg="green", width=11, height=4)
			btnChooseOne.option, btnChooseOne.btnChooseOne, btnChooseOne.selected, btnChooseOne.colorOrig = option, self, 0, "green"
	...
	def resolveMove(self, entity, button, selectedSubject):
		if self.UI == 0:
			...
			if selectedSubject == "ChooseOneOption" and entity.available():
				...
				for i in range(len(self.Game.options)):
					if entity == self.Game.options[i]:
						self.printInfo("Choice {} chosen".format(self.Game.options[i].name))
						self.UI, self.choice = 2, i #选项已准备好，进入目标或打出位置选择界面选择界面
						for button in self.buttonsDrawn:
							if type(button) == ChooseOneButton:
								button.remove()
						break
			else:
				self.printInfo("You must click an option to continue.")
			...
			if self.selectedSubject.endswith("onBoard"):
			...
			#随从的打出位置和抉择选项已经在上一步选择，这里处理目标选择。
				...
				if selectedSubject.endswith("MiniononBoard") == False: #指定目标时必须是英雄或者场上随从
	...
	def waitforDiscover(self):
		...
		btnDiscoverHide = DiscoverHideButton(relief=tk.FLAT, master=self.GamePanel, text="Hide\nOptions", bg="green", width=8, height=3, font=("Yahei", 10, "bold"))
		...
		btnDiscoverHide.place(x=0.8*X, y=0.5*Y, anchor='c')
		...
		for i in range(len(self.Game.options)):
			...
			img = img.resize((70, 70))
			ph = PIL.ImageTk.PhotoImage(img)
			btnDiscover = DiscoverOptionButton(relief=tk.FLAT, master=self.GamePanel, image=ph, bg="green", width=CARD_X, height=CARD_Y)
