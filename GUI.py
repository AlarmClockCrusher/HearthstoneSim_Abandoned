...
def findPicFilepath(card):
	if card.cardType != "Hero" and card.cardType != "Hero Power":
		path = "Crops\\%s\\"%card.index.split('~')[0]
	else:
		path = "Crops\\HerosandPowers\\"
	if "Mutable" in type(card).__name__:
		cardName = type(card).__name__.split("_")[0]
	else:
		cardName = type(card).__name__
	filepath = path+"%s.png"%cardName
	return filepath
...
class GUI:
		...
		if self.UI == -1: #Draw the mulligan part, the cards and the start turn button
			...
			if self.UI == 1:
				...
				for i in range(len(self.Game.minions[1])):
					self.posMinionsDrawn[1].append((0.043*X+(X/8)*i, 0.58*Y))
	...
	def drawHeroesWeaponsPowers(self):
		for ID in range(1, 3):
			...
			ph = PIL.ImageTk.PhotoImage(img)
			btnHero = HeroButton(relief=tk.FLAT, master=self.GamePanel, image=ph, bg=color, width=Hand_X, height=1.2*Hand_Y)
			btnHero.image = ph
