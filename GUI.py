...
class GUI_1P(GUI_Common):
	def __init__(self):
		...
		self.boardID = makeCardPool(monkVar.get(), self.boardID.get())
		self.window.destroy()
		self.window = tk.Tk()
		...
		self.output = tk.Listbox(master=self.outPutPanel, xscrollcommand=scrollbar_hor.set, yscrollcommand=scrollbar_ver.set, width=35, height=9, bg="white", font=("Yahei", 13))
