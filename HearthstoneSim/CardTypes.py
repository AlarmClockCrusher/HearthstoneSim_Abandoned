...
class Card:
	...
	def selfManaChange(self):
		if self.cardType == "Minion" and self.keyWords["Echo"] > 0 and self.mana < 1:
			self.mana = 1
		elif self.cardType == "Spell" and "Echo" in self.index and self.mana < 1:
			self.mana = 1
	...
	def getsFrozen(self):
		if self.onBoard or self.inHand:
			...
			self.sendSignal(self.cardType+"Frozen", self.Game.turn, None, self, 0, "")
	...
	def leavesHand(self):
		...
		for trigger in self.triggersinHand: #将手牌扳机全部清空。
			trigger.disconnect()
			#If the trigger is temporary, it will be removed once it leaves hand, whether being discarded, played or shuffled into deck
			if trigger.temp:
	...
	def leavesDeck(self):
		self.onBoard, self.inHand, self.inDeck = False, False, False
		self.mana = self.mana_set
		for trigger in self.triggersinDeck: #将牌库扳机全部清空。
			trigger.disconnect()
	...
	def attacks(self, target, consumeAttackChance=True):
		...
		if damageDealer_attacker.cardType != "Hero" and damageDealer_attacker.marks["Attack Adjacent Minions"] > 0 and target.cardType == "Minion":
			...
			for minion in adjacentMinions:
				...
				if objtoTakeDamage.damageRequest(damageDealer_attacker, subject_attack) > 0:
					damageActual, survial = objtoTakeDamage.takesDamage(damageDealer_attacker, subject_attack, sendDamageSignal=False)
					damageDealingList.append((damageDealer_attacker, objtoTakeDamage, damageActual))
...
class Minion(Card):
	...
	def blank_init(self, Game, ID):
		...
		self.name = type(self).name
		self.mana, self.mana_set = type(self).mana, type(self).mana
		self.attack = type(self).attack
		self.health, self.health_upper = type(self).health, type(self).health
		self.tempAttackChanges = [] #list of (tempAttChange, expiration timepoint)
		#The stat of a minion are classified as:
			#type(self).attack; self.attack_Enchant; self.attack(affected by buffAura and tempChange)
			#type(self).attack; self.health_Enchant; self.health, self.health_upper.
	...
	#The dies() method is completely obsolete, as game will handle the death of minions.
	def deathResolution(self, attackbeforeDeath):
		print("Now resolving the death of ", self.name)
		self.Game.sendSignal("MinionDies", self.Game.turn, None, self, attackbeforeDeath, "")
		...
		self.Game.sendSignal("MinionDied", self.Game.turn, None, self, 0, "")
	...
	#结算阶段
		if self.magnetic > 0:
			...
			if magneticTarget != None and magneticTarget.onBoard and self.ID == magneticTarget.ID:
				...
				minion = self.magnetCombine(magneticTarget)
	...
	#完成阶段
		...
		return minion #如果随从打出过程中发生了变形，则需要根据返回值决定后续处理。
	...
	def selfCopy(self, ID, attack=False, health=False, mana=False):
		...
		#If the card is added from board to hand, the mana_set is equal to its original mana cost.
	...
	def getsSilenced(self):
		if self.onBoard:
			...
			#Remove the enrage/Lightspawn/Paragon of Light effects. And discard effect.
			...
			self.statReset(type(self).attack, type(self).health)
...
class Spell(Card):
	...
	def blank_init(self, Game, ID):
		...
		self.onBoard, self.inHand, self.inDeck = False, False, False
		self.identity = [np.random.rand(), np.random.rand(), np.random.rand()]
		self.triggersinHand, self.triggersinDeck = [], []
	...
	def cast(self, target=None, comment="CastbyOthers"):
		...
		for i in range(repeatTimes):
			if self.overload > 0:
				...
				self.Game.ManaHandler.overloadMana(self.overload, self.ID)
			if self.twinSpell > 0:
				self.Game.Hand_Deck.addCardtoHand(self.index.replace("-Twinspell", "") + "-Uncollectible", self.ID, "CreateUsingIndex")
		...
		#暂时先不考虑尤格萨隆的谜之匣对星界密使的消耗。
		self.Game.sendSignal("SpellBeenCast", self.Game.turn, self, target, 0, "CastbyOthers", choice=0)
	...
	def played(self, target=None, choice=0, mana=0, comment=""):
		...
		self.Game.target = target
		self.Game.sendSignal("SpellTargetDecision", self.ID, self, target, 0, choice)
		target = self.Game.target
	...
	#完成阶段：
		...
		if "-Echo" in self.index:
			echo = type(self)(self.Game, self.ID)
			echo.turnEndTrigger(EchoCardDisappear)
...
class Secret(Spell):
	...
	def cast(self, target=None, comment="CastbyOthers"):
		...
		#使用后步骤，但是此时的扳机只会触发星界密使和风潮的状态移除，因为其他的使用后步骤都要求是玩家亲自打出。
		self.Game.sendSignal("SpellBeenPlayed", self.ID, self, None, 0, "CastbyOthers")
	...
	def whenEffective(self, target=None, comment="", choice=0):
		if self.Game.SecretHandler.isSecretDeployedAlready(self, self.ID) == False:
...
class Hero(Card):
	...
	def blank_init(self, Game, ID):
		self.Game = Game
		self.mana_set, self.mana = type(self).mana, type(self).mana
	...
	def getsHealed(self, subject, heal, sendHealSignal=True):
		if self.health == self.health_upper:
			...
			return 0, 0 #healDone is 0, survial status is 0
		else:
...
class Weapon(Card):
	...
	def deathResolution(self, attackbeforeDeath):
		...
		self.Game.sendSignal("WeaponDestroyed", self.ID, None, self, 0, "")
	...
	#结算阶段
		...
		if self.Game.playerStatus[self.ID]["Battlecry Trigger Twice"] > 0:
			...
			weaponPlayed, target = self.whenEffective(target, 0)
		weaponPlayed, target = self.whenEffective(target, 0)
		#消灭旧武器，并将列表前方的武器全部移除。
