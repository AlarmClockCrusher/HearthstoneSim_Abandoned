...
X = 800
Y = 700
BoardPosition = (0, 0.3*Y, 0.85*X, 0.45*Y)
Hero1Position = (X/2-30, Y-165, 60, 80)
Hero2Position = (X/2-30, 110, 60, 80)
EndTurnPosition1 = (X-100, Y/2+40, 50, 30)
EndTurnPosition2 = (X-100, Y/2-40, 50, 30)
EndMulliganPosition = (X/2-10, Y/2, 50, 30)
Weapon1Position = (X/2-100, Y-150, 60, 60)
Weapon2Position = (X/2-100, 130, 60, 60)
HeroPower1Position = (X/2+50, Y-140, 60, 60)
HeroPower2Position = (X/2+50, 130, 60, 60)
...
class GUI:
	def __init__(self):
		...
		self.posHands = {1:[], 2:[]}
		self.posOptions = []
		self.posMulligans = {1:[(10+(X/10)*i, Y - 65, 40, 60) for i in range(len(self.Game.mulligans[1]))],
							2:[(10+(X/10)*i, 20, 40, 60) for i in range(len(self.Game.mulligans[2]))]}
	...
	def update(self):
		...
		self.drawHeroes()
		pygame.draw.rect(self.display, blue, BoardPosition, 4)
		...
		for i in range(len(self.Game.minions[1])):
			self.posMinionsDrawn[1].append((30+(X/8)*i, Y-270, 60, 80))
		for i in range(len(self.Game.minions[2])):
			self.posMinionsDrawn[2].append((30+(X/8)*i, 250, 60, 80))
		#Draw the minions
		self.drawMinions()
		
		#Decide the posHands, where to draw the hands.
		for i in range(len(self.Game.Hand_Deck.hands[1])):
			self.posHands[1].append((10+(X/10)*i, Y - 65, 40, 60))
		for i in range(len(self.Game.Hand_Deck.hands[2])):
			self.posHands[2].append((10+(X/10)*i, 20, 40, 60))
		self.drawHands()
		
		self.drawManasHandsDecksSecrets()	
		self.drawHeroPowers()
		self.drawWeapon()
	...
	def drawMinions(self):
		for ID in range(1, 3):
			for i in range(len(self.posMinionsDrawn[ID])):
				pos = self.posMinionsDrawn[ID][i]
				if self.Game.minions[ID][i].cardType == "Minion":
					color = green if self.Game.minions[ID][i].canAttack() else red
					pygame.draw.rect(self.display, color, pos)
					self.displayText(self.Game.minions[ID][i].name, blue, (pos[0]+pos[2]/2, pos[1]))
					self.displayText((str)(self.Game.minions[ID][i].attack), blue, (pos[0], pos[1]+pos[3]))
					color = blue if self.Game.minions[ID][i].health >= self.Game.minions[ID][i].health_upper else red
					self.displayText((str)(self.Game.minions[ID][i].health), color, (pos[0]+pos[2], pos[1]+pos[3]))
				else:
					pygame.draw.rect(self.display, red, pos)
					self.displayText(self.Game.minions[ID][i].name, blue, (pos[0]+pos[2]/2, pos[1]))
					
				self.displayText("Sequence", blue, (pos[0]+pos[2]/2-5, pos[1]+pos[3]-15))
				self.displayText((str)(self.Game.minions[ID][i].sequence), blue, (pos[0]+pos[2], pos[1]+pos[3]-15))
				self.displayText("Position", blue, (pos[0]+pos[2]/2-5, pos[1]+pos[3]-30))
				self.displayText((str)(self.Game.minions[ID][i].position), blue, (pos[0]+pos[2], pos[1]+pos[3]-30))
		...
		for ID in range(1, 3):
			for i in range(len(self.posHands[ID])):
				...
				if ID == self.Game.turn and card.mana <= self.Game.ManaHandler.manas[ID]:
					if card.cardType == "Spell" and card.available():
						color = green
					elif card.cardType == "Minion" and self.Game.spaceonBoard(ID) > 0:
						color = green
					elif card.cardType == "Weapon" or card.cardType == "Hero":
						color = green
						
				pos = self.posHands[ID][i]
				pygame.draw.rect(self.display, color, pos)
				self.displayText(card.name, blue, (pos[0]+pos[2]/2, pos[1]))
	...
	def drawHeroPowers(self):
		...
		self.displayText(self.Game.heroPowers[1].name, black, (HeroPower1Position[0]+HeroPower1Position[2]/2, HeroPower1Position[1]+HeroPower1Position[3]/2))
		self.displayText(self.Game.heroPowers[2].name, black, (HeroPower2Position[0]+HeroPower2Position[2]/2, HeroPower2Position[1]+HeroPower2Position[3]/2))
		
	def drawManasHandsDecksSecrets(self): #Draw the manas and the size of hands and decks
		if self.Game.turn == 1:
			color1, color2 = black, red
		else:
			color1, color2 = red, black
			
		self.displayText("Mana:%d/%d"%(self.Game.ManaHandler.manas[1], self.Game.ManaHandler.manasUpper[1]), color1, (X-60, Y-130), 20)
		self.displayText("Overload: %d"%self.Game.ManaHandler.manasOverloaded[1], color1, (X-70, Y-200), 20)
		self.displayText("Locked: %d"%self.Game.ManaHandler.manasLocked[1], color1, (X-70, Y-220), 20)
		self.displayText("Hand:%d"%len(self.Game.Hand_Deck.hands[1]), color1, (X-60, Y-150), 20)
		self.displayText("Deck:%d"%len(self.Game.Hand_Deck.decks[1]), color1, (X-60, Y-170), 20)
		
		self.displayText("Mana:%d/%d"%(self.Game.ManaHandler.manas[2], self.Game.ManaHandler.manasUpper[2]), color2, (X-60, 100), 20)
		self.displayText("Overload: %d"%self.Game.ManaHandler.manasOverloaded[2], color2, (X-70, 160), 20)
		self.displayText("Locked: %d"%self.Game.ManaHandler.manasLocked[2], color2, (X-70, 180), 20)
		self.displayText("Hand:%d"%len(self.Game.Hand_Deck.hands[2]), color2, (X-60, 120), 20)
		self.displayText("Deck:%d"%len(self.Game.Hand_Deck.decks[2]), color2, (X-60, 140), 20)
		
		for i in range(len(self.Game.SecretHandler.secrets[1])):
			self.displayText(self.Game.SecretHandler.secrets[1][i].name, color2, (0.75*X, Y-110-i*16), 15)
			
		for i in range(len(self.Game.SecretHandler.secrets[2])):
			self.displayText(self.Game.SecretHandler.secrets[2][i].name, color1, (0.75*X, 80+i*16), 15)
	...
	def shutDown(self, comment):
		self.gameOver = True
		dispatcher.send("GUIShutDown", ID=0, subject=None, target=None, number=0, comment="", sender=self.Game)
		exit()
