...
class Deathrattle_Minion(TriggeronBoard):
	...
	def selfCopy(self, newMinion): #用于无面操纵者等复制场上随从时亡语的复制。
		return type(self)(newMinion) #巫毒娃娃等需要记录一定信息的亡语会有自己的专有函数
...
class Deathrattle_Weapon(TriggeronBoard):
	...
	def selfCopy(self, newWeapon):
		return type(self)(newWeapon)
...
class SecretTrigger(TriggeronBoard):
	...
	def connect(self):
		for signal in self.signals:
			print("Secret %s no registers trigger"%self.entity, self, signal)
			self.entity.Game.triggersonBoard[self.entity.ID].append((self, signal))
