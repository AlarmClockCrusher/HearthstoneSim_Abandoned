...
class Card:
    ...
    def leavesHand(self, intoDeck=False):
        ...
        for trig in reversed(self.trigsBoard):
            trig.disconnect()
            if hasattr(trig,
                       "temp") and trig.temp:  # If the trig is temporary, it will be removed once it leaves hand, whether being discarded, played or shuffled into deck
                try:
    ...
    def leavesDeck(self, intoHand=True):
        ...
        for trigger in self.trigsBoard:
            trigger.disconnect()
            if hasattr(trigger,
                       "temp") and trigger.temp:  # If the trigger is temporary, it will be removed once it leaves hand, whether being discarded, played or shuffled into deck
    ...
    def selectablebyBattle(self, subject):
        ...
        return self.onBoard and self.ID != subject.ID and self.status["Temp Stealth"] < 1 \
               and ((self.type == "Minion" and self.status["Immune"] + self.keyWords["Stealth"] + self.marks[
            "Can't Be Attacked"] < 1) \
                    or (self.type == "Hero" and self.Game.status[self.ID]["Immune"] + self.Game.status[self.ID][
                    "Hero Can't Be Attacked"] < 1) \
                    ) \
               and (self.Game.status[subject.ID]["Ignore Taunt"] > 0 or (
                subject.type == "Minion" and subject.marks["Ignore Taunt"]) \
                    # 随从在作为攻击目标时如果自己有嘲讽则随机无须判定是否有其他嘲讽
                    or (self.type == "Minion" and self.keyWords["Taunt"] > 0) \
                    # 在攻击方没有无视嘲讽的情况下，且目标不是一个有嘲讽的随从时，需要友方其他随从中没有嘲讽
                    or not any(
                    minion.keyWords["Taunt"] > 0 and minion.status["Temp Stealth"] + minion.status["Immune"] +
                    minion.keyWords["Stealth"] < 1 for minion in self.Game.minionsonBoard(self.ID)) \
                    )
            ...
            selectable = target.onBoard \
                         and ( \
                                     (targetType == "Hero" and (
                                             ...
                                             target.status["Temp Stealth"] + target.marks[
                                                 "Enemy Effect Evasive"] < 1) \
                                     ...
                                     target.ID == self.ID or target.marks["Enemy Effect Evasive"] < 1) \
                                         and not ((self.type == "Power" or self.type == "Spell") and target.marks[
                                         "Evasive"] > 1)) \
    ...
    def selectableEnemyMinionExists(self, choice=0):
        return any(
            minion.type == "Minion" and minion.onBoard and self.canSelect(minion) and self.targetCorrect(minion, choice) \
    ...
    def selectableEnemyAmuletExists(self, choice=0):
        return any(
            minion.type == "Amulet" and minion.onBoard and self.canSelect(minion) and self.targetCorrect(minion, choice) \
            for minion in self.Game.minions[3 - self.ID])
    ...
    def selectableFriendlyExists(self, choice=0):  # For minion battlecries, the friendly hero is always selectable
        ...
        return (self.type != "Spell" or self.type != "Power") \
               or (self.canSelect(hero) and self.targetCorrect(hero, choice) or self.selectableFriendlyMinionExists(
            choice))
    ...
    def selectionLegit(self, target, choice=0):
        ...
        PRINT(self.Game,
              "Verifying the validity of selection. Subject {}, target {} with choice {}".format(self.name, target,
                                                                                                 choice))
        if target:  # 指明了目标
            ...
            return not self.needTarget(choice) or ((self.type == "Minion" or self.type == "Amulet") and not (
                    self.needTarget(choice) and self.targetExists(choice)))
    ...
    def attacks(self, target, useAttChance=True):
        ...
        if damageDealer_attacker.type != "Hero" and damageDealer_attacker.marks[
            "Sweep"] > 0 and target.type == "Minion":
    ...
    def dealsDamage(self, target, damage):
        if target.onBoard or target.inHand:
            ...
            return target, 0  # The target is neither on board or in hand. Either removed already or shuffled into deck.
    ...
    def restoresAOE(self, targets, heals):
        ...
        if self.Game.status[self.ID]["Heal to Damage"] > 0:
            ...
            for target, healActual in zip(targets_healed, healsConnected):
                self.Game.sendSignal(target.type + "GetsHealed", self.Game.turn, self, target, healActual,
                                     "FullyHealed")
                if target.type == "Hero":
                    containHero = True
            if containHero:
    ...
    def hardCopy(self, ID):
        ...
        for key, value in self.__dict__.items():
            ...
            elif type(value) == list or type(value) == dict or type(
                    value) == tuple:  # If the attribute is a list or dictionary, use the method defined at the start of py
...
class Spell(Card):
        ...
        if curGame.mode == 0:
            if curGame.guides:
                ...
                if self.chooseOne < 1:
                    ...
                    choice = -1 if curGame.status[self.ID]["Choose Both"] else nprandint(len(self.options))
                if target is None:  # 沼泽女王哈加莎的恐魔是目前唯一的指定发动的
                    if self.needTarget(choice):
                        targets = self.findTargets("", choice)[0]
                        if targets:
                            if "enemy1st" in comment:
                                enemies = [char for char in targets if
                                           self.canSelect(char) and self.targetCorrect(char, choice)]
                                target = npchoice(enemies) if enemies else npchoice(targets)
                            elif "targetPrefered" in comment and preferedTarget:
                                target = preferedTarget if self.canSelect(preferedTarget) and self.targetCorrect(
                                    preferedTarget, choice) else npchoice(targets)
                            else:
                ...
                if target:
                    if target.type == "Hero":
                        i, where = target.ID, "hero"
                    else:
                        i, where = target.position, "minion%d" % target.ID
                else:
                    i, where = 0, ''
    ...
    def played(self, target=None, choice=0, mana=0, posinHand=-2, comment=""):
        ...
        self.Game.sendSignal("SpellPlayed", self.ID, self, target if not isinstance(target, list) else None, mana, "",
                             choice)
        # 获得过载和双生法术牌。
        if self.overload > 0:
        ...
        for i in range(repeatTimes):
            if i == 1:  # 第二次施放时照常获得过载和双生法术牌。
                ...
                if self.twinSpell > 0:
                    PRINT(self.Game,
                          "Twinspell %s is cast and adds a another copy to player's hand, pos" % (self.name, posinHand))
                    self.Game.Hand_Deck.addCardtoHand(self.twinSpellCopy, self.ID, "type", posinHand)
            ...
            if target and target.type == "Minion" and target.onBoard and sweep > 0 and self.Game.neighbors2(target)[0]:
                ...
                PRINT(self.Game,
                      "{} will also be cast upon minions {} adjacent to the target {} with choice {}".format(self.name,
                                                                                                             neighbors,
                                                                                                             target.name,
                                                                                                             choice))
    ...
    def countHealDouble(self):
        return sum(minion.marks["Heal x2"] > 0 or minion.marks["Spell Heal&Dmg x2"] > 0 for minion in
                   self.Game.minions[self.ID])
...
class HeroPower(Card):
    ...
    def STATUSPRINT(self):
        ...
        PRINT(self.Game,
              "Chances_base: %d. Chances_extra %d" % (self.heroPowerChances_base, self.heroPowerChances_extra))
        if self.manaMods != []:
            PRINT(self.Game, "\tCarries mana modification:")
            for manaMod in self.manaMods:
                if manaMod.changeby != 0:
    ...
    def use(self, target=None, choice=0):
        ...
        if self.Game.Manas.affordable(self) == False:
            ...
            if self.available() and self.selectionLegit(target, choice):
                ...
                PRINT(self.Game,
                      "Invalid selection to use Hero Power {} on target {}, with choice {}".format(self.name, target,
                                                                                                   choice))

        if canUseHeroPower:
            PRINT(self.Game,
                  "*********\nHandling using Hero Power {} with target {}, with choice	{}\n*********".format(
                      self.name, target, choice))
            # 支付费用，清除费用状态。
            ...
            if target and target.type == "Minion" and self.Game.status[self.ID]["Power Sweep"] > 0:
                ...
                if targets != []:
                    PRINT(self.Game, "%s will also be cast upon minions adjacent to the target minion %s" % (
    ...
    def countHealDouble(self):
        return sum(minion.marks["Heal x2"] > 0 or minion.marks["Power Heal&Dmg x2"] > 0 for minion in
                   self.Game.minions[self.ID])
...
class Hero(Card):
    ...
    def STATUSPRINT(self):
        PRINT(self.Game, "Game is {}.".format(self.Game))
        PRINT(self.Game,
              "Hero %d %s: Attacked times: %d.	Base att chances left: %d.	Extra att chances left: %d" % (
                  self.ID, self.name, self.attTimes, self.attChances_base, self.attChances_extra))
        if self.manaMods != []:
            PRINT(self.Game, "\tCarries mana modification:")
            for manaMod in self.manaMods:
                if manaMod.changeby != 0:
                    ...
                    PRINT(self.Game, "\t\tChanged to %d" % manaMod.changeto)
        PRINT(self.Game, "\tHero %s is frozen	{},  has Temp Stealth {}".format(self.status["Frozen"],
                                                                                  self.status["Temp Stealth"]))
