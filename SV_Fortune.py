    ...
    def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
        ...
        if comment == 6:
            if self.Game.mode == 0:
                t = None
                if self.Game.picks:
                    t = self.Game.cardPool[self.Game.picks.pop(0)]
                else:
                    minions = {}
...
class Trig_CatTuner(TrigBoard):
    ...
    def effect(self, signal, ID, subject, target, number, comment, choice=0):
        ...
        if curGame.mode == 0:
            t = None
            if curGame.picks:
                t = curGame.cardPool[curGame.picks.pop(0)]
            else:
                minions = {}
...
class PrinceofCocytus_Accelerate(SVSpell):
    ...
    def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
        ...
        if curGame.mode == 0:
            types = []
            if curGame.picks:
                types = list(curGame.picks.pop(0))
            else:
                while len(types) < 4:
                    t = npchoice(cards)
                    if t not in types:
