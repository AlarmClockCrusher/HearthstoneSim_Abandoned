...
class WiseLorewalkerCho(Minion): #睿智的游学者周卓
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if self.Game.Hand_Deck.handNotFull(self.ID) and self.ID == self.Game.turn:
			...
			#需要把所有的职业捋一遍，出一个dict
			mana = self.Game.ManaHandler.manas[self.ID]
			...
			if "InvokedbyOthers" in comment:
				card = np.random.choice(np.random.choice(list(spells.keys())))
				PRINT(self, "Wise Lorewalker Cho's battlecry adds a random spell from another class with cost equal player's remain mana to player's hand")
				self.Game.Hand_Deck.addCardtoHand(card, self.ID, "CreateUsingType")
			else:
				classes = list(spells.keys())
				classes = np.random.choice(classes, min(3, len(classes)), replace=False)
				for Class in classes:
					self.Game.options.append(np.random.choice(spells[Class])(self.Game, self.ID))
				PRINT(self, "Wise Lorewalker Cho's battlecry lets player discover a Dragon")
...
class DefenseTechniqueMaster(Minion): #御术僧师
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if posinHand > -2 and not self.Game.ManaHandler.manas[self.ID]:
			PRINT(self, "Defense Technique Master's Quaff triggers and gives the minion 'Deathrattle: Give your hero +3 Attack this turn'")
...
class GiveYourHeroPlus3AttackThisTurn(Deathrattle_Minion):
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		PRINT(self, "Deathrattle: Give your hero +3 Attack this turn triggers.")
...
class PurifyingBrew(Spell): #活血酒
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		PRINT(self, "Purifying Brew lets player draw a card")
		card = self.Game.Hand_Deck.drawCard(self.ID)[0]
		if card and posinHand > -2 and not self.Game.ManaHandler.manas[self.ID] and self.Game.Hand_Deck.handNotFull(self.ID):
			PRINT(self, "Purifying Brew's Quaff triggers and adds a copy of the drawn card to player's hand")
			self.Game.sendSignal("QuaffTriggered", self.ID, None, None, 0, "")
...
class Transcendance(Spell): #魂体双分
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if target != None and self.Game.Hand_Deck.handNotFull(self.ID):
			PRINT(self, "Transcendance is cast and adds a copy of minion %s to player's hand"%target.name)
...
class Liquor(Spell): #醉酿
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if target != None:
			if posinHand > -2 and not self.Game.ManaHandler.manas[self.ID]: #Triggers Quaff
				PRINT(self, "Liquor's Quaff triggers and destroys minion %s"%target.name)
				self.Game.sendSignal("QuaffTriggered", self.ID, None, None, 0, "")
				target.dead = True
			elif target.onBoard: #假设只有当目标随从还在场上的时候会生效
				PRINT(self, "Liquor gives renders minion %s unable to attack for two turns"%target.name)
...
class Trigger_Liquor(TriggeronBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		self.counter -= 1
		if self.counter < 1:
			PRINT(self, "%s can attack next turn."%self.entity.name)
			self.entity.marks["Can't Attack"] -= 1
			self.disconnect()
			extractfrom(self, self.entity.triggersonBoard)
		else:
			PRINT(self, "%s still can't attack for another turn"%self.entity.name)
...
class StaveOff(Spell): #酒不离手
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		PRINT(self, "Stave Off is cast and player draws two Quaff cards from the deck.")
		for i in range(2):
			quaffCardsinDeck = []
			...
			if quaffCardsinDeck:
				self.Game.Hand_Deck.drawCard(self.ID, np.random.choice(quaffCardsinDeck))
		return None
...
class TouchofKarma(Spell): #业报之触
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if target != None and target.onBoard:
			PRINT(self, "Touch of Karma is cast and damage on player's Hero this turn will go to enemy minion %s instead"%target.name)
			self.Game.DamageHandler.scapegoatforHero[self.ID] = target
...
class Trigger_TouchofKarma(TriggeronBoard):
	...
	def connect(self):
		for signal in self.signals:
			self.entity.Game.triggersonBoard[self.entity.ID].append((self, signal))
		self.entity.Game.DamageHandler.scapegoatforHero[self.ID] = self.entity
		
	def disconnect(self):
		for signal in self.signals:
			extractfrom((self, signal), self.entity.Game.triggersonBoard[self.entity.ID])
		if self.entity.Game.DamageHandler.scapegoatforHero[self.ID] == self.entity:
			self.entity.Game.DamageHandler.scapegoatforHero[self.ID] = None
...
class JadeTether(HeroPower):
	...
	def effect(self, target, choice=0):
		if target:
			PRINT(self, "Hero Power Jade Tether transforms %s into a 2/2 Golem with Taunt"%target.name)
...
class Trigger_JadeTether(TriggeronBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		...
		PRINT(self, "Player uses Hero Power and Jade Tether has been used for %d times"%self.entity.progress)
		if self.entity.progress > 1:
			PRINT(self, "Player has used Hero Power Jade Tether twice and the Hero Power changes back to the original one it replaced")
...
class DrunkenBoxing(Spell): #醉拳
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		PRINT(self, "Drunken Boxing gives player +5 Attack this turn. It randomly attacks all minions")
		hero = self.Game.heroes[self.ID]
		hero.gainTempAttack(5)
		quaffTriggered = (posinHand > -2 and self.Game.ManaHandler.manas[self.ID] == 0)
		minionstoAttack = self.Game.minionsonBoard(1) + self.Game.minionsonBoard(2)
		np.random.shuffle(minionstoAttack)
		if quaffTriggered:
		...
		while minionstoAttack:
			...
			if attackTarget.health > 0 and not attackTarget.dead and hero.health > 0 and not hero.dead:
				...
				self.Game.battleRequest(hero, attackTarget, False, True, False, False)
...
class OnimaActuary(Minion): #秘典宗精算师
	...
	mana, attack, health = 4, 4, 4
	index = "Classic~Monk~Minion~4~4~4~Monk~Onima Actuary~Stealth"
...
class Trigger_OnimaActuary(TriggeronBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		PRINT(self, "After %s attacks, player's Hero Power costs (1) for the rest of turn"%self.entity.name)
		tempAura = YourHeroPowerCosts1ThisTurn(self.entity.Game, self.entity.ID)
		self.entity.Game.ManaHandler.PowerAuras.append(tempAura)
		tempAura.auraAppears()
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		if hdHandler.handNotFull(self.ID) and self.Game.turn == self.ID:
			...
			for card in hdHandler.decks[self.ID]:
				if card.mana == 1 and card.cardType == "Minion": oneCostMinions.append(card)
			if oneCostMinions:
				if "InvokedbyOthers" in comment or len(oneCostMinions) < 2:
					PRINT(self, "Spawn of Xuen's battlecry lets player draw a random 1-Cost minion from deck")
					minion = np.random.choice(oneCostMinions)
					self.Game.Hand_Deck.drawCard(self.ID, minion)
				else:
					PRINT(self, "Spawn of Xuen's battlecry lets player discover a 1-Cost minion from deck")
					self.Game.options = np.random.choice(oneCostMinions, min(3, len(oneCostMinions)), replace=False)
					self.Game.DiscoverHandler.startDiscover(self)
			else: PRINT(self.Game, "No 1-Cost minion in deck. Spawn of Xuen's battlecry has no effect")
		return None
		
	def discoverDecided(self, option):
		PRINT(self, "Player draws 1-Cost minion %s"%option.name)
...
class QuickSip(Spell): #浅斟快饮
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if self.Game.Hand_Deck.decks[self.ID]:
			if (posinHand == -2 or self.Game.ManaHandler.manas[self.ID]) or "InvokedbyOthers" in comment or len(self.Game.Hand_Deck.decks[self.ID]) == 1:
				PRINT(self, "Quick Sip adds 3 copies of a random card in player's deck to player's hand"%target.name)
				card = np.random.choice(self.Game.Hand_Deck.decks[self.ID])
				self.Game.Hand_Deck.addCardtoHand([card.selfCopy(self.ID) for i in range(3)], self.ID)
			else: #Triggers Quaff
				PRINT(self, "Quick Sip's Quaff triggers and lets player discover a card from the deck to add 3 copies of it to player's hand")
				self.Game.sendSignal("QuaffTriggered", self.ID, None, None, 0, "")
	...
	def discoverDecided(self, option):
		PRINT(self, "Quick Sip puts 3 copies of card %s into player's hand"%option.name)
...
class ShaohaotheEmperor(Minion): #皇帝 少昊
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		PRINT(self, "Shaohao, the Emperor's battlecry give the minion Stealth until player's next turn")
...
#抽一张牌。如果你的生命值大于或等于15点，则为你的英雄恢复所有生命值
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		PRINT(self, "Inner Peace is cast and lets player draw a card")
		self.Game.Hand_Deck.drawCard(self.ID)
		if self.Game.heroes[self.ID].health > 14:
			PRINT(self, "Player's health is 15 or higher, and Inner Peace fully heals the player")
...
class RushingJadeWind(Spell): #碧玉疾风
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		PRINT(self, "Rushing Jade Wind deals %d damage to all enemy minions"%damage)
		self.dealsAOE(targets, [damage for minion in targets])
		return None
...
class Trigger_KunLaiSummitZenAlchemist(TriggeronBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		...
		if enemyMinions:
			enemyMinion = np.random.choice(enemyMinions)
			PRINT(self, "At the end of turn, %s changes the Attack of random enemy minion %s to 1."%(self.entity.name, enemyMinion.name))
...
class Trigger_FistsoftheHeavens(TriggeronBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		PRINT(self, "Player's Quaff triggers and %s gains +4 Durability."%self.entity.name)
...
class MonkDragonKeeper(Minion): #驭龙武僧
	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		if posinHand > -2 and self.Game.ManaHandler.manas[self.ID] == 0:
			PRINT(self, "Monk Dragon Keeper's Quaff triggers and gives the minion Taunt and 'Deathrattle: Shuffle this into your deck'")
			self.Game.sendSignal("QuaffTriggered", self.ID, None, None, 0, "")
			self.getsKeyword("Taunt")
