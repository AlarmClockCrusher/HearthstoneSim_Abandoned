...
class SpinemUp_Option:
	...
	def available(self):
		return self.entity.Game.spaceonBoard(self.entity.ID)
...
class Trigger_Chenvaala(TriggeronBoard):
	...
	def effect(self, signal, ID, subject, target, number, comment, choice=0):
		if signal == "SpellBeenPlayed":
			self.counter += 1
			PRINT(self, "Player casts a spell and %s records its current progress to summon a 5/5 Elemental: %d"%(self.entity.name, self.counter))
			if self.counter > 0 and self.counter % 3 == 0:
				PRINT(self, "Player has cast %d spells, %s summons a 5/5 Elemental"%(self.counter, self.entity.name))
