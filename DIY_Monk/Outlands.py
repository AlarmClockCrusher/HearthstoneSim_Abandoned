...
class ImprisonedDormantForm(Permanent):
	...
	def createCopy(self, recipientGame):
		if self in recipientGame.copiedObjs:
			...
			Copy = type(self)(recipientGame, self.ID, self.originalMinion.createCopy(recipientGame))
		...
		if self.Game.turn != self.ID or self.Game.CounterHandler.numSpellsPlayedThisTurn[self.ID] % 3 != 2:
			...
			for trig in self.triggersonBoard:
				if type(trig) == Trigger_KaelthasSunstrider:
					trig.counter = self.Game.CounterHandler.numSpellsPlayedThisTurn[self.ID]
